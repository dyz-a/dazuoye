...
## 项⽬概述
本项目基于 MuJoCo MPC 物理仿真框架，实现了汽车仪表盘可视化系统。核心功能为将自定义仪表盘 UI 整合到 3D 仿真环境中，通过读取 MuJoCo 的实时仿真数据（速度、位置、转速等），以 2D 覆盖层形式渲染速度表（0-200km/h）、转速表（0-8000RPM）、油量及温度显示等核心模块，数据随车辆运动实时更新。同时支持基础美化与功能扩展，如指针平滑动画、超限警告提示，可选小地图、油耗计算等进阶功能，实现了物理仿真数据与可视化界面的高效联动，为车辆仿真提供直观的状态监控工具。
